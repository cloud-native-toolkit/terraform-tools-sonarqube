...
resource "null_resource" "sonarqube_helm" {
  ...
  provisioner "local-exec" {
    command = "${self.triggers.bin_dir}/helm upgrade -i ${self.triggers.name} ${local.chart_dir} -n ${self.triggers.namespace}"
  ...
  provisioner "local-exec" {
    when    = destroy
    command = "${self.triggers.bin_dir}/helm uninstall ${self.triggers.name} -n ${self.triggers.namespace}"
